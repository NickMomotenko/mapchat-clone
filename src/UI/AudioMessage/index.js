...
const AudioMessage = () => {
  ...
  let durationRef = React.useRef(null);
  let playerRef = null;
  ...
  return (
    <AudioMessageWrapp
      center
      width={generateWidthFromSeconds(playerRef?.audioEl.current.currentTime)}
    >
      <ReactAudioPlayer
        ref={(element) => {
          playerRef = element;
        ...
        onLoadedMetadata={() => {
          setDuration(secondsToMinutes(playerRef?.audioEl.current.duration));
        }}
        onListen={() => {
          setCurrentTime(
            secondsToMinutes(playerRef?.audioEl.current.currentTime)
          );
        }}
      />
